// Compute status color at render time to avoid regenerating data on theme change
export const getStatusColor = (theme, statusText) => {
  switch (statusText) {
    case "In Progress":
      return "#8A8CD9";
    case "Complete":
    ...
    case "Approved":
      return "#FFC555";
    case "Rejected":
      return theme === "dark" ? "#FFFFFF66" : "#1C1C1C66";
    default:
      return theme === "dark" ? "#FFFFFF66" : "#1C1C1C66";
  }
};

// Deterministic, theme-agnostic dataset (no randomness, no theme dependency)
export const ordersData = Array.from({ length: 50 }, (_, i) => {
  const statuses = [{ text: "In Progress" }, { text: "Complete" }, { text: "Pending" }, { text: "Approved" }, { text: "Rejected" }];
